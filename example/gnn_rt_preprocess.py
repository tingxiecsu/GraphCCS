...
#Transform the above each data of numpy to pytorch tensor on a device (i.e., CPU or GPU).
    ...
    return dataset
def create_dataset_randomsplit_training(x,y,path): 
    dataset = []  
...
def create_dataset_randomsplit_tf(x,y,path,path2):
    dir_input = path + 'SMRT-'
    ...
    for i in range(len(x)):
        ...
        if '.' in smiles:
            continue
        #smiles = 'InChI=1S/C12H19NO4/c1-2-3-4-5-9(14)8-11(15)13-10-6-7-17-12(10)16/h10H,2-8H2,1H3,(H,13,15)'        
        mol = Chem.MolFromSmiles(smiles)
        #mol = Chem.MolFromInchi(smiles)
        if mol is None:       
            continue     
...
#Transform the above each data of numpy to pytorch tensor on a device (i.e., CPU or GPU).
        fingerprints = torch.LongTensor(fingerprints).to(device)
        adjacency = torch.FloatTensor(adjacency).to(device)
        propertys = torch.FloatTensor([[float(propertys)]]).to(device)

        dataset.append((smiles,fingerprints, adjacency, molecular_size, propertys))
    dir_dataset=path2+ 'tf1-'
    dump_dictionary(fingerprint_dict, dir_dataset + '-fingerprint_dict.pickle')
    dump_dictionary(atom_dict, dir_dataset+ '-atom_dict.pickle')
    dump_dictionary(bond_dict, dir_dataset + '-bond_dict.pickle')
    dump_dictionary(edge_dict, dir_dataset + '-edge_dict.pickle')
...
def create_dataset_kfold(x,y,path,path2,dataname):
    dir_input =path+'SMRT-'
    with open(dir_input + 'atom_dict.pickle', 'rb') as f:
        c=pickle.load(f)
        for k in c.keys():
...
def transferlearning_dataset_predict(y,path):
    dir_input = path + 'training--'
    with open(dir_input + 'atom_dict.pickle', 'rb') as f:
        c=pickle.load(f)
        for k in c.keys():
            atom_dict.get(k)
